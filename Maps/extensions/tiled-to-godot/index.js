...
const computeGodotTileModifier = (cell) => {
    /**
     ...
     * rotated 3x right
     */
    if (cell.flippedHorizontally === false &&
...
const buildTileMapNodeString = (layer) => {
    const data = [];
...
const buildTileMapSceneString = (map) => {
    ...
    for (let layerIndex = 0; layerIndex < map.layerCount; layerIndex++) {
        ...
        if (layer.isTileLayer && layer.visible) {
            scene.push(buildTileMapNodeString(layer));
        }
    }
    return scene.join('\n');
};
const visitCell = (layer, it) => {
    const rect = layer.region().boundingRect;
    for (let i = 0; i <= rect.width; i++) {
        for (let j = 0; j <= rect.height; j++) {
            const x = rect.x + i;
...
const buildSimulationData = (map) => {
    ...
    visitObjectLayer(map, (layer) => {
        if (!DefaultPlayerSpawn) {
            const obj = layer.objects.find((obj) => ((obj.shape === MapObject.Point) &&
...
var Walkable;
(function (Walkable) {
    Walkable[Walkable["UNDEFINED"] = 0] = "UNDEFINED";
    Walkable[Walkable["NO"] = 1] = "NO";
    Walkable[Walkable["YES"] = 2] = "YES";
})(Walkable || (Walkable = {}));
const getWalkableFromTile = (tile) => {
...
tiled.registerMapFormat('GodotRoguelike', {
    ...
    write: (map, scenePath) => {
        try {
            ...
            // @ts-ignore
            const extension = FileInfo.suffix(scenePath);
